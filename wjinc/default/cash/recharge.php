...
<div class="pagemain">
    ...
    <div class="display biao-cont">
        ...
        <?php
		//$sql="select * from {$this->prename}bank_list b, {$this->prename}admin_bank m where m.admin=1 and m.enable=1 and b.isDelete=0 and b.id=m.bankId";
		//$sql="select * from {$this->prename}bank_list b, {$this->prename}admin_bank m where m.enable=1 and b.isDelete=0 and b.id=m.bankId";
		$sql="select * from {$this->prename}bank_list where isDelete=0";
		$banks=$this->getRows($sql);
		//print_r($banks);
				if($banks){?>
				  <form action="/index.php/cash/pay" method="post" target="_blank">
			...
			<?php
				$set=$this->getSystemSettings();
							...
							<div class="bankchoice">
								<label><input value="<?=$bank['id']?>" type="radio" name="mBankId" data-bank='<?=json_encode($bank)?>' style="width:auto;" /><span style="background:url(/<?=$bank['logo']?>);"
									><?=$bank['name']?></span></label>
							</div>
							<?php } ?>
		</dd></dl>
       <dl class="wjform">
