...
<table width="100%" class='table_b'>
	...
	<?php } ?>
        ...
        <?php if($para['userType']==1 || ($para['userType']==0 && !$para['parentId']) ||($para['username'] && $para['username']!='用户名')){//------------------------------------------
				$sql2="select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from lottery_members u left join lottery_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere $userWhere3";
		...
		<?php }else{//----------------------------------------
				$sql2="select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from lottery_members u left join lottery_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere $userWhere";
				$all=$this->getRow($sql2);
